...
document.querySelector("#sub").addEventListener("click", () => {
      ...
      count++;
        li.innerHTML = `
                    ...
                    <div class="is">
                      ...
                      <i class="nedit fi fi-rr-edit under editee" id="edit${count}" onclick="edite()"></i>
  ...
  function edite() {
    ...
    var elid = clicked.id;
    var id = '';
    for (let y = 4; y < elid.length; y++) {
      id = id + elid[y];
    }
    document.querySelector(`#task${id}`).classList.remove("trans");
    document.querySelector(`#task${id}`).classList.add("place");
    document.querySelector(`#task${id}`).removeAttribute("disabled");
    document.querySelector(`#edit${id}`).classList.remove("under");
    document.querySelector(`#del${id}`).classList.remove("under");
    document.querySelector(`#edit${id}`).classList.add("disgard");
    document.querySelector(`#del${id}`).classList.add("disgard");
    document.querySelector(`#heart${id}`).classList.remove("under");
    document.querySelector(`#heart${id}`).classList.add("disgard");
    document.querySelector(`#check${id}`).classList.remove("disgard");
    document.querySelector(`#check${id}`).classList.add("under");
    document.querySelector(`#cross${id}`).classList.add("under");
    val = document.querySelector(`#task${id}`).value;
    document.querySelector(`#task${id}`).focus();
    document.querySelector(`#task${id}`).style.color = "#000";
    updatedata();
}
...
function updatedata(){
    ...
    .then(() => {
        alert('The data updated successfuly');
    })
    .catch((error) => {
